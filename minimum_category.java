...
public class minimum_category extends HttpServlet{
...
public void getCategories()
{
	try{
			...
			Document doc = dBuilder.parse(new URL("http://services.e-prices.gr/getCategoriesWS?ui="+ui).openStream());
					...
					if(!(fstNm.item(0)).getNodeValue().equals("306")){
						...
						if(!(fstNm.item(0)).getNodeValue().equals("985")|| lock3 == 1){
							...
							if(!(fstNm.item(0)).getNodeValue().equals("986") || lock4 == 1){
								...
								element1 = (Element) nodelist.item(0);
								fstNm = element1.getChildNodes();
								String valueISO = fstNm.item(0).getNodeValue();//new String((fstNm.item(0)).getNodeValue().getBytes("UTF-8"), "ISO-8859-1");
...
public void doPost(HttpServletRequest request,HttpServletResponse response)throws ServletException, IOException{
	...
	if (act.equals("minimum")){
		getCategories();	
		try{

			request.setAttribute("category", category);
			request.getRequestDispatcher("/min_cat.jsp").forward(request, response);
